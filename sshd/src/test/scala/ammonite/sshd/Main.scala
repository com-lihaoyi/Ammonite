...
object Main {
  def main(args: Array[String]): Unit = {
    val config =
      SshServerConfig("localhost", 2222, currentUserName, getPassword)
    val ammoniteServer = new SshdRepl(config)

    ammoniteServer.start()
    println(
      s"Ammonite server started. $helpMessage." +
  ...
  @tailrec private def exitAwaitLoop(): Unit =
    System.console().readLine() match {
