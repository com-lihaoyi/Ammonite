...
object Compiler extends CompilerBuilder {
  ...
  def apply(classpath: Seq[java.net.URL],
            ...
            lineNumberModifier: Boolean = true): ICompiler = new ICompiler{
    ...
    var lastImports = Seq.empty[Imports.Data]
     ...
     */
    def compileOrNull(src: Array[Byte],
                      printer: Logger,
                      importsLen0: Int,
                      userCodeNestingLevel: Int,
                      fileName: String): ICompiler.Output = {
      ...
      this.errorLogger = printer.printError
      this.warningLogger = printer.printWarning
      this.infoLogger = printer.printInfo
      ...
      if (reporter.hasErrors) null
        ...
        val imports = lastImports.toArray
        new ICompiler.Output(
          files.toArray.map { case (s, b) => entry(s, b) },
          new Imports(imports),
          usedEarlierDefinitions.map(_.toArray).orNull
        )
