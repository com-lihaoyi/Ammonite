...
class WhiteListClasspath(aggregates: Seq[ClassPath], whitelist: Set[Seq[String]])
  ...
  override def packages(inPackage: PackageName) =
    super.packages(inPackage)

  override def classes(inPackage: PackageName) =
    super.classes(inPackage).filter { t =>
      Util.lookupWhiteList(whitelist, inPackage.dottedString.split('.') ++ Seq(t.name + ".class"))
    }

  override def sources(inPackage: PackageName) =
    super.sources(inPackage).filter { t =>
      Util.lookupWhiteList(whitelist, inPackage.dottedString.split('.') ++ Seq(t.name + ".class"))
    }

  override def hasPackage(pkg: PackageName): Boolean =
    super.hasPackage(pkg)*/
