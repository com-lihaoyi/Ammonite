...
trait Storage {
  ...
  def classFilesListSave(
      filePathPrefix: os.SubPath,
...
object Storage {
  ...
  case class InMemory() extends Storage {
    ...
    def classFilesListLoad(filePathPrefix: os.SubPath, cacheTag: Tag): Option[ScriptOutput] = {
  ...
  class Folder(val dir: os.Path, isRepl: Boolean = true) extends Storage {
    ...
    def compileCacheSave(path: String, tag: Tag, data: CompileCache): Unit = {
      val tagCacheDir = {
        compileCacheDir / path.split('.').map(
          encode
        ) / tag.code / tag.env / tag.classPathWhitelistHash
      }
