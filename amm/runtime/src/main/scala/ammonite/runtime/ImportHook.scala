...
trait ImportHook {
  ...
  def handle(source: CodeSource,
             ...
             wrapperPath: Seq[Name]): Either[String, Seq[ImportHook.Result]]
}
...
object ImportHook {
  ...
  def resolveFiles(tree: ImportTree,
    ...
    val relative =
      tree.prefix
        .map {
          case ammonite.util.Util.upPathSegment => os.up; case x => os.rel / x
        }
        .reduce(_ / _)
    ...
    def handle(source: CodeSource,
      ...
      source.path match {
        ...
        case Some(currentScriptPath) =>
          ...
          if (missing.nonEmpty) {
            Left(
              "Cannot resolve $file import: " + missing
                .map(_ + ".sc")
                .mkString(", "))
          } else {
            Right(
  ...
  class BaseIvy(plugin: Boolean) extends ImportHook {
    ...
    def resolve(
      ...
      val splitted = for (signature <- signatures) yield {
        signature.split(':') match {
          ...
          case Array(a, "", b, c) =>
            Right(
              Dependency.of(a, b + "_" + IvyConstructor.scalaBinaryVersion, c))
          case Array(a, "", "", b, c) =>
            Right(
              Dependency
                .of(a, b + "_" + IvyConstructor.scalaFullBinaryVersion, c))
    ...
    def handle(source: CodeSource,
               tree: ImportTree,
               interp: InterpreterInterface,
               wrapperPath: Seq[Name]): Either[String, Seq[Result.ClassPath]] =
      for {
        ...
        Seq(
          Result.ClassPath(Some(deps), resolved.map(os.Path(_)).toSeq, plugin))
  }
