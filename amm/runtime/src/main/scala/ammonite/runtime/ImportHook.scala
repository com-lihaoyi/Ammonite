...
object ImportHook {
  ...
  object Http extends ImportHook {
    ...
    def handle(source: ImportHook.Source,
      ...
      tree.mappings match {
        ...
        case Some(mappings) =>
          Res.map(tree.mappings.get) {
            case (k, v) => resolveHttp(k, v.getOrElse(k))
          }
      }
    }
  }
  ...
  class BaseIvy(plugin: Boolean) extends ImportHook {
    ...
    def resolve(interp: InterpreterInterface, signature: String) =
      for {
        (a, b, c) <- signature.split(':') match {
          case Array(a, b, c) => Res.Success((a, b, c))
          case Array(a, "", b, c) =>
            Res.Success((a, b + "_" + IvyThing.scalaBinaryVersion, c))
    ...
    def handle(source: ImportHook.Source,
      ...
      source match {
        case Source.File(currentScriptPath) =>
          ...
          if (missing.nonEmpty) {
            Res.Failure(None,
                        "Cannot resolve $cp import: " + missing.mkString(", "))
          } else
            Res.Success(
              for (((relativeModule, rename), filePath) <- relativeModules.zip(
                     files))
