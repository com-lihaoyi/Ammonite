...
class EvaluatorImpl(currentClassloader: ClassLoader,
                    startingLine: Int,
                    timer: Timer) extends Evaluator{ eval =>
  ...
  val userCodeExceptionHandler: PartialFunction[Throwable, Res.Failing] = {
    ...
    case Ex(e: ThreadDeath)                 =>   EvaluatorImpl.interrupted(e)

    // Interrupted during evaluation
    case Ex(_: InvEx, e: ThreadDeath)       =>   EvaluatorImpl.interrupted(e)
