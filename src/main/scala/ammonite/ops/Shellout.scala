...
object Shellout {
  ...
  def executeInteractive(wd: Path, cmd: Command[_]) = {
    ...
    val proc =
      builder.command(cmd.cmd: _*).inheritIO().start()
  ...
  def executeStream(wd: Path, cmd: Command[_]) = {
    ...
    builder.directory(new java.io.File(wd.toString))
    val process =
      builder.command(cmd.cmd: _*).start()
...
case class CommandResult(exitCode: Int, chunks: Seq[Either[Bytes, Bytes]]) {
  ...
  override def toString() = {
    s"CommandResult $exitCode\n" +
      chunks.iterator.collect {
        case Left(s) => s
        case Right(s) => s
      }.map(x => new String(x.array)).mkString
  }
}
...
object Shellable {
  ...
  implicit def BasePathShellable(s: BasePath): Shellable =
    Shellable(Seq(s.toString))
