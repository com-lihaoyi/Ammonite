...
trait CustomInterpreter extends IMain{
  import formatting._
  ...
  private class WrappedRequest(val req: Request) extends CompiledScript {
    ...
    def getEngine: ScriptEngine = CustomInterpreter.this
  }
  ...
  private def requestFromLine(line: String, synthetic: Boolean): Either[IR.Result, Request] = printing{
