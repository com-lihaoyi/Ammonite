...
import scala.tools.nsc.util.{ScalaClassLoader, StackTraceOps, stringFromReader, stringFromWriter}
...
/** An interpreter for Scala code.
  ...
  *  @author Lex Spoon
  */
class IMainX(@BeanProperty val factory: ScriptEngineFactory, initialSettings: Settings, protected val out: JPrintWriter)
  extends AbstractScriptEngine with Compilable with ImportsX {
  imain =>
  ...
  private[ammonite] var totalSilence               = false     // whether to print anything
  ...
  import global._
  import definitions.{ObjectClass, dropNullaryMethod, termMember}
  ...
  private def withLastExceptionLock[T](body: => T, alt: => T): T = {
    ...
    catch   {case _ => ???} //logAndDiscard("withLastExceptionLock", alt)
  ...
  lazy val isettings = new ISettingsX(this)
...
//    settings.explicitParentLoader.getOrElse( this.getClass.getClassLoader() )
  ...
  */
  ...
  def replwarn(msg: => String)    = if (isReplDebug) echo(msg)
      ...
      case Left(result) => result match {
        case IR.Incomplete => {
          ...
          new CompiledScript {
            ...
            def getEngine: ScriptEngine = IMainX.this
          }
        }
        case _ => {
          code = ""
  ...
  class Request(val line: String, val trees: List[Tree]) {
    ...
    abstract class Wrapper extends IMainX.CodeAssembler[MemberHandler] {
    ...
    private object ResultObjectSourceCode extends IMainX.CodeAssembler[MemberHandler] {
  ...
  def createBindings: Bindings = new IBindingsX {
  ...
  def runtimeClassAndTypeOfTerm(id: String): Option[(JClass, Type)] = {
    implicit def f[T](c: Class[T]) = new scala.tools.nsc.interpreter.RichClass(c)
  ...
  object exprTyper extends {
    val repl: IMainX.this.type = imain
  } with ExprTyperX { }
...
object IMainX {
  ...
  class Factory extends ScriptEngineFactory {
    ...
    def getScriptEngine: ScriptEngine = new IMainX(this, new Settings() {
  ...
  class ReplStrippingWriter(intp: IMainX) extends StrippingTruncatingWriter(intp.out) {
    import intp._
