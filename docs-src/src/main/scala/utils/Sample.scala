...
//    val ammOutput = lines.split("\u001b")
  ...
  def exec(command: Seq[String],
           input: String,
           args: Map[String, String] = Map.empty): String =
    cached(("exec", command, input, args)) {
      ...
      println(
        s"====================RESULT ${p.exitValue()}===================="
      )
      println(result)
      println("========================================")
      assert(p.exitValue() == 0,
             "Non-zero exit value for subprocess: " + p.exitValue())
  ...
  def compare(bashCode: String, ammoniteCode: String) = {
    val out = {
      ...
      for (chunk <- output.split("\n" + customPrompt, -1).drop(1).dropRight(1))
        yield {
