...
class SessionApiImpl(eval: Evaluator){
...
class ReplApiImpl(interp: Interpreter,
  ...
  lazy val replApi: ReplAPI = new DefaultReplAPI { outer =>
    ...
    object sess extends Session {
      def frames = eval.frames
      def save(name: String) = sess.save(name)
      def delete(name: String) = sess.delete(name)

      def pop(num: Int = 1) = {
        val res = sess.pop(num)
        reInit()
        res
      }
      def load(name: String = "") = {
        val res = sess.load(name)
        reInit()
        res
      }
    }
